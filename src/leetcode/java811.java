    ...
    static class Solution {
        public List<String> subdomainVisits(String[] cpdomains) {
            ...
            for(String temp:cpdomains){
                ...
                int num=Integer.valueOf(temp.substring(0,flag));
                String address=temp.substring(flag+1);
                //判断地址中'.'的次数
                int count = address.length()-address.replace(".","").length();
                for(int i=0;i<=count;i++){
                    //判断map中是否有这个地址，有的话value值累加上
                    if (ans.containsKey(address)){
                        ans.put(address,ans.get(address)+num);
                    }else {ans.put(address,num);}
                    address= address.substring(address.indexOf('.')+1);
                }
//                int test= address.indexOf('.');
